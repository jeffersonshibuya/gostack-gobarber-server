...
let UpdateProfileService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    if (checkUserEmail && checkUserEmail.id !== user_id) throw new _AppError.default('E-mail already registred');
    if (password && !old_password) throw new _AppError.default('You should inform the password and the old password');
